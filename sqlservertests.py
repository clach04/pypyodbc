...
class SqlServerTestCase(unittest.TestCase):
    ...
    def test_autocommit(self):
        ...
        othercnxn = pypyodbc.connect(self.connection_string, autocommit=True)
    ...
    def test_cursorcommit(self):
        ...
        othercnxn = pypyodbc.connect(self.connection_string)
    ...
    def test_unicode_results(self):
        ...
        othercnxn = pypyodbc.connect(self.connection_string, unicode_results=True)
        othercursor = othercnxn.cursor()
    ...
    def test_sqlserver_callproc(self):
        try:
            self.cursor.execute("drop procedure pypyodbctest")
            self.cnxn.commit()
        except:
            pass
        ...
        self.cursor.execute("""
                            create procedure pypyodbctest @var1 varchar(32)
                            as 
                            begin 
                              select s 
                              from t1 
        ...
        # for row in self.cursor.procedureColumns('pypyodbctest'):
        #     print row.procedure_name, row.column_name, row.column_type, row.type_name

        self.cursor.execute("exec pypyodbctest 'hi'")
    ...
    def test_none_param(self):
        ...
        #
        # If SQLDescribeParam doesn't work, pypyodbc would use VARCHAR which almost always worked.  However,
        ...
        row = self.cursor.execute("select * from t1").fetchone()
        self.assertEqual(row[0], 1)
        self.assertEqual(type(row[1]), bytearray)
    ...
    def test_output_conversion(self):
        def convert(value):
            ...
            return 'X' + value + 'X'
        self.cnxn.add_output_converter(pypyodbc.SQL_VARCHAR, convert)
    ...
    def test_login_timeout(self):
        ...
        cnxns = pypyodbc.connect(self.connection_string, timeout=2)
    ...
    def test_context_manager_success(self):
        ...
        with pypyodbc.connect(self.connection_string) as cnxn:
            cursor = cnxn.cursor()
    ...
    def test_cursor_context_manager_success(self):
        ...
        with pypyodbc.connect(self.connection_string).cursor() as cursor:
            cursor.execute("insert into t1 values (1)")
